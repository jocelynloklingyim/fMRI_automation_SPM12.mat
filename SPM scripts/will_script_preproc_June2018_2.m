...
for sub = subs;                                                             % subject loop
    ...
    clear matlabbatch
   matlabbatch{1}.spm.spatial.normalise.estwrite.subj.resample{1,1} = spm_select('FPList',fullfile(directories.rdir,directories.wdir,directories.sdir,folders{end}),'^r.*\.nii$');
    for ses = 1:sesnum
        imgs = spm_select('FPList',fullfile(directories.rdir,directories.wdir,directories.sdir,folders{ses}),'^ar.*\.nii$');
        imgs = mat2cell(imgs,ones(size(imgs,1),1),size(imgs,2));
        for vol = 1:length(imgs)
            matlabbatch{1}.spm.spatial.normalise.estwrite.subj.resample{end+1,1} = imgs{vol};
        end
    end
    matlabbatch{1}.spm.spatial.normalise.estwrite.subj.source = {spm_select('FPList',fullfile(directories.rdir,directories.wdir,directories.sdir,folders{end}),'^r.*\.nii$')};
    matlabbatch{1}.spm.spatial.normalise.estwrite.subj.wtsrc = '';
    matlabbatch{1}.spm.spatial.normalise.estwrite.eoptions.template = {'C:\Users\ling9\Downloads\spm12\spm12\toolbox\OldNorm\T1.nii,1'};  %THIS NEEDS TO BE CHANGED FOR YOUR COMPUTER
